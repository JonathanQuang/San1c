 ...
 * QSort(arr): 
 * QuickSort basically starts by putting one integer in
   order, than putting the left side of the int and right side in 
   order as well, by making a web-like pattern of partitionsorts till
   everything is in place.
 * 2a. Worst pivot choice / array state and associated runtime: 
 * Mixed up, not in order, most random.
 * 2b. Best pivot choice / array state and associated runtime:
 * Largest to shortest, or vice versa (in order).
 * 3. Approach to handling duplicate values in array:
 * Use if statements with <= or >=
 *****************************************************/
...
public class QuickSort 
{
	...
	if(left < right){
	    int x = partition( d, left, right, (left + right)/2);	
	qsort( d, left, x-1);
	qsort( d, x+1, right);
	}
    }
